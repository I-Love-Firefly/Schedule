...
public static class OcrLayoutSerializer
{
    public const string Instruction =
        "你是课程表 OCR 版面结构化器。输入包含设备 OCR 得到的文本块，每行格式为 [x0,y0,x1,y1] 文本，坐标已归一化到 0..1000。" +
...
public static class AiScheduleJsonParser
{
    ...
    public static AiScheduleDocument Parse(string output)
    {
        ...
        if (parsed.SchemaVersion != 1) throw new InvalidDataException("API 模型返回了不支持的数据版本。 ");
        if (parsed.Courses.Count > 60) throw new InvalidDataException("API 模型输出课程数量异常，已拒绝导入。 ");
        return parsed;
    }
    ...
    private static AiScheduleDocument RecoverCompleteCourseObjects(string candidate)
    {
        ...
        if (courses.Count == 0) throw new InvalidDataException("API 模型没有返回完整课程对象。 ");
